...
test_that("Test filter_by_species() for error", {
  expect_error(
    filter_by_species(df_cpr),
    "This function is not designed to work with 'CPR North' data",
    fixed = TRUE
  )

  expect_error(
    filter_by_species(df),
    paste0(
      "This function is not designed to work on long format",
      " data.frame"
    ),
    fixed = TRUE
  )
  ...
  expect_error(
    filter_by_species(df[, 1, drop = FALSE]),
    "No species columns detected",
    fixed = TRUE
  )

  expect_error(
    filter_by_species(df_net),
    "Argument 'species' is required",
    fixed = TRUE
  )

  expect_error(
    filter_by_species(df_net, species = 1),
    "Argument 'species' must be a character of length >= 1",
    fixed = TRUE
  )

  expect_error(
    filter_by_species(df_net, species = "toto"),
    "The species provided are absent from 'data'",
    fixed = TRUE
  )

  expect_error(
    filter_by_species(df_net, species = c("toto", "titi")),
    "The species provided are absent from 'data'",
    fixed = TRUE
  )
})
