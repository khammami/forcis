...
forcis_mcp_help <- function(topic = "overview") {
  switch(
    ...
    "database" = paste(
      ...
      "",
      "Species data is organized into three taxonomy levels:",
      "- LT (Lumped Taxonomy): 47 coarser, simplified taxa. Best for",
      "  broad community-level analyses.",
      "- VT (Validated Taxonomy): 56 detailed taxa including subspecies",
      "  variants. Recommended for most analyses.",
      "- OT (Original Taxonomy): 203 taxa preserving the original lab",
      "  naming conventions from diverse data sources.",
      "",
      "Use get_species_names() to list all known species, or filter by",
      "taxonomy level (e.g., get_species_names(taxonomy = 'VT')).",
      "",
    ...
    "package" = paste(
      ...
      "- get_species_names(data): Extract species names from a loaded",
      "  data.frame's column names. (For a canonical list of all known",
      "  species without loading data, use the MCP get_species_names tool.)",
      "",
      "Computation:",
